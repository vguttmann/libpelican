...
//     Nick Kirkby for having signaled me this point, see https://github.com/pierremolinaro/CAN2518/issues/5);
...
static void myESP32Task(void* pData) {
    CAN2518FD* canDriver = (CAN2518FD*)pData;
    while (1) {
...
CAN2518FD::CAN2518FD(const uint8_t inCS, // CS input of MCP2517FD
...
uint32_t CAN2518FD::begin(const CAN2518FDSettings& inSettings,
    void (*inInterruptServiceRoutine) (void)) {
    //--- Add pass-all filter
    CAN2518FDFilters filters;
...
uint32_t CAN2518FD::begin(const CAN2518FDSettings& inSettings,
    ...
    if (inFilters.filterStatus() != CAN2518FDFilters::kFiltersOk) {
    ...
    if (errorCode == 0) {
        ...
        case CAN2518FDSettings::OSC_4MHz:
        case CAN2518FDSettings::OSC_20MHz:
        case CAN2518FDSettings::OSC_40MHz:
            break;
        case CAN2518FDSettings::OSC_4MHz_DIVIDED_BY_2:
        case CAN2518FDSettings::OSC_20MHz_DIVIDED_BY_2:
        case CAN2518FDSettings::OSC_40MHz_DIVIDED_BY_2:
            osc = 1 << 4; // Divide by 2
            break;
        case CAN2518FDSettings::OSC_4MHz10xPLL_DIVIDED_BY_2:
            pll = 1; // Enable 10x PLL
            osc = 1 << 4; // Divide by 2
            break;
        case CAN2518FDSettings::OSC_4MHz10xPLL:
            pll = 1; // Enable 10x PLL
            break;
        }
        osc |= pll;
        if (inSettings.mCLKOPin != CAN2518FDSettings::SOF) {
    ...
    if (errorCode == 0) {
        ...
        mTXQBufferPayload = CAN2518FDSettings::objectSizeForPayload(inSettings.mControllerTXQBufferPayload);
        ...
        mReceiveFIFOPayload = CAN2518FDSettings::objectSizeForPayload(inSettings.mControllerReceiveFIFOPayload);
        ...
        mTransmitFIFOPayload = CAN2518FDSettings::objectSizeForPayload(inSettings.mControllerTransmitFIFOPayload);
        //----------------------------------- Configure receive filters
        uint8_t filterIndex = 0;
        CAN2518FDFilters::Filter* filter = inFilters.mFirstFilter;
...
#ifdef ARDUINO_ARCH_ESP32
        xTaskCreate(myESP32Task, "CAN2518Handler", 1024, this, 256, &mESP32TaskHandle);
#endif
...
bool CAN2518FD::tryToSend(const CANFDMessage& inMessage) {
...
bool CAN2518FD::enterInTransmitBuffer(const CANFDMessage& inMessage) {
    bool result;
...
uint32_t lengthCodeForLength(const uint8_t inLength) {
...
void CAN2518FD::appendInControllerTxFIFO(const CANFDMessage& inMessage) {
...
bool CAN2518FD::sendViaTXQ(const CANFDMessage& inMessage) {
    bool ok = mUsesTXQ;
    if (ok) {
...
bool CAN2518FD::receive(CANFDMessage& outMessage) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
bool CAN2518FD::dispatchReceivedMessage(const tFilterMatchCallBack inFilterMatchCallBack) {
...
void CAN2518FD::isr_poll_core(void) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#endif
...
void CAN2518FD::transmitInterrupt(void) { // Generated if hardware transmit FIFO is not full
...
void CAN2518FD::receiveInterrupt(void) {
...
void CAN2518FD::writeRegister32Assume_SPI_transaction(const uint16_t inRegisterAddress,
...
void CAN2518FD::writeRegister8Assume_SPI_transaction(const uint16_t inRegisterAddress,
...
uint32_t CAN2518FD::readRegister32Assume_SPI_transaction(const uint16_t inRegisterAddress) {
    //--- Read word register via 6-byte buffer (speed enhancement, thanks to thomasfla)
    uint8_t buffer[6] = { 0 };
    //--- Enter command
...
uint16_t CAN2518FD::readRegister16Assume_SPI_transaction(const uint16_t inRegisterAddress) {
...
uint8_t CAN2518FD::readRegister8Assume_SPI_transaction(const uint16_t inRegisterAddress) {
...
void CAN2518FD::writeRegister8(const uint16_t inRegisterAddress, const uint8_t inValue) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
uint8_t CAN2518FD::readRegister8(const uint16_t inRegisterAddress) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
uint16_t CAN2518FD::readRegister16(const uint16_t inRegisterAddress) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
void CAN2518FD::writeRegister32(const uint16_t inRegisterAddress, const uint32_t inValue) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
uint32_t CAN2518FD::readRegister32(const uint16_t inRegisterAddress) {
    mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
    taskDISABLE_INTERRUPTS();
#else
...
CAN2518FDSettings::OperationMode CAN2518FD::currentOperationMode(void) {
    const uint8_t mode = readRegister8(CON_REGISTER + 2) >> 5;
    return CAN2518FDSettings::OperationMode(mode);
}

//······················································································································

bool CAN2518FD::recoverFromRestrictedOperationMode(void) {
    bool recoveryDone = false;
    if (currentOperationMode() == CAN2518FDSettings::RestrictedOperation) { // In Restricted Operation Mode
...
void CAN2518FD::setOperationMode(const CAN2518FDSettings::OperationMode inOperationMode) {
    //  bits 7-4: Transmit Bandwith Sharing Bits ---> 0
    //  bit 3: Abort All Pending Transmission bits --> 0
...
void CAN2518FD::reset2517FD(void) {
...
bool CAN2518FD::performSleepModeToConfigurationMode(void) {
...
uint32_t CAN2518FD::errorCounters(void) {
    return readRegister32(TREC_REGISTER);
}

//----------------------------------------------------------------------------------------------------------------------

uint32_t CAN2518FD::diagInfos(const int inIndex) { // thanks to Flole998 and turmary
...
void CAN2518FD::gpioSetMode(const uint8_t inPin, const uint8_t inMode) {
    if (inPin <= 1) {
...
void CAN2518FD::gpioWrite(const uint8_t inPin, const uint8_t inLevel) {
    if (inPin <= 1) {
...
bool CAN2518FD::gpioRead(const uint8_t inPin) {
...
void CAN2518FD::configureGPIO0AsXSTBY(void) {
    uint8_t value = readRegister8(IOCON_REGISTER_00_07);
